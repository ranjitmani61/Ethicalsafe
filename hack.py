...
from modules.motion import detect_motion
...
def main():
    ...
    tool = st.sidebar.selectbox("Choose a tool", [
        ...
        "Motion Detection", "Packet Capture", "File Hash Check", "Port Scan", 
    ...
    if tool == "Device Info":
        ...
        if st.button("Detect Motion"):
            with results:
                ...
                st.json(result)
                logger.info(f"Motion Detection: {result}")
                if "file" in result:
                    st.image(result["file"])
